...
const addPurchases = (arr) => {
  return arr.reduce((accum, current) => {
    return accum + current.purchasePrice;
  }, 0);
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((accum, current) => {
    if (current.children) {
      return accum + current.children.length;
    } else {
      return accum;
    }
  }, 0);
...
const calculateAverage = (arr) => {
  ...
  return result.number / result.count;
};
...
const extractStats = (snorlaxData) => {
  return snorlaxData.stats.reduce((acc, current) => {
    acc[current.stat.name] = current.baseStat;
    return acc;
  }, {});
};
